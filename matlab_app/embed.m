...
% Ybutton=imread('1.png');   % button图片
% set(handles.pushbutton3,'CData',Ybutton);  % button图片
% set(handles.pushbutton4,'CData',Ybutton);  % button图片
% Bbutton=imread('4.png');   % button图片
% set(handles.pushbutton1,'CData',Bbutton);  % button图片
% set(handles.pushbutton2,'CData',Bbutton);  % button图片
% global flag_hi;
% flag_hi=0;
% global flag_wi;
% flag_wi=0;
handles.flag_hi=0; % 标记 查看是否读入了host Img
guidata(hObject,handles); % Update handles structure
handles.flag_wi=0; % 标记 查看是否读入了水印图像WImg
guidata(hObject,handles); % Update handles structure
handles.flag_wdi=0;% 标记 查看是否生成了atermarked Img
guidata(hObject,handles); % Update handles structure
...
axes(handles.axes1); % 选择显示图像的axes
wImg=[pathname filename]; % 水印图像路径
img_src=imread(wImg); % 读入水印图像
handles.wImg=img_src; % 水印图像存储到handles中 % Update handles structure
handles.flag_wi=1; % 水印图读取标记变为1
guidata(hObject,handles); % Update handles structure
imshow(img_src); % 展示水印图像
...
axes(handles.axes2);% 选择显示图像的axes
hostImage=[pathname filename]; % 宿主图像路径
img_src=imread(hostImage); % 读取宿主图像
handles.hostImage=img_src; % 将宿主图像存储到handles
handles.flag_hi=1;% 更改宿主图像标记
guidata(hObject,handles); % Update handles structure
imshow(img_src); % 展示宿主图像


% --- Executes on button press in pushbutton3.
function pushbutton3_Callback(hObject, eventdata, handles)

wname=handles.wname; % 取数据
lambda=handles.lambda; % 取数据
dt=handles.dt; % 取数据
n=handles.n; % 取数据
a=handles.a; % 取数据
b=handles.b; % 取数据
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
flag_wi = handles.flag_wi; % 取水印图像标记
flag_hi = handles.flag_hi; % 取宿主图像标记
% 根据标记返回不同的结果
...
w1 = dsh_embed2(ht1,wImg,s1,block_size1,lambda,wname,n,a,b); % 嵌入鲁棒水印w1
watermarkedImg(:,:,1) = w1; % 嵌入后替换
...
w2 = dsh_embed3(ht2,wImg,s1,block_size2,dt,wname); %   嵌入鲁棒水印w2 
watermarkedImg(:,:,2) = w2; % 嵌入后替换s
...
watermarkedImg = dsh_embedFragileW(watermarkedImg,block_size3); % 嵌入脆弱水印w3 
...
handles.watermarkedImg=watermarkedImg; % 将水印图像存储到handles
guidata(hObject,handles); % Update handles structure
handles.flag_wdi=1; % 更改含水印图像标记
guidata(hObject,handles);% Update handles structure
...
if flag_wdi==0
	box=msgbox('no watermarked Image','error','error'); % 没watermarked Image 
else
watermarkedImg=handles.watermarkedImg; % 取watermarkedImg
T = clock;
...
imwrite(watermarkedImg,path) % 将watermarkedImg保存
box = msgbox('Saved successfully!','msg','help'); % 保存成功提示
waitfor(box); 
close(gcbf); 
close(gcbf); 
embed;
...
box=msgbox('submitted','msg','help'); % 提交成功
